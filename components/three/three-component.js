...
export class ThreeComponent {
    ...
    return `
            <div class="bg-white shadow-sm p-4 mx-auto" style="max-width: 1100px; border: 1px solid #dee2e6;">
                <div id="viewer-controls" class="d-flex gap-2 mb-3 justify-content-center">
                    <button id="zoom-in" class="btn btn-outline-secondary btn-sm"><i class="bi bi-plus-lg"></i></button>
                    <button id="zoom-out" class="btn btn-outline-secondary btn-sm"><i class="bi bi-dash-lg"></i></button>
                    <div class="vr mx-2"></div>
                    <button id="view-front" class="btn btn-sm btn-light border">Спереди</button>
                    <button id="view-back" class="btn btn-sm btn-light border">Сзади</button>
                    <button id="view-left" class="btn btn-sm btn-light border">Слева</button>
                    <button id="view-right" class="btn btn-sm btn-light border">Справа</button>
                </div>
  ...
  initThree(data) {
    ...
    // Функция для добавления модели в сцену
    const addModelToScene = (gltf, xOffset = 0) => {
      const obj = gltf.scene;

      // Базовое центрирование каждой модели
      const box = new THREE.Box3().setFromObject(obj);
      const center = box.getCenter(new THREE.Vector3());

      obj.position.x = xOffset - center.x;
      obj.position.z = -center.z;
      obj.position.y = -box.min.y; // Ставим на "пол"

      this.scene.add(obj);
    };

    // ЛОГИКА ЗАГРУЗКИ (как в карточке, но для большой сцены)
    if (data.models && Array.isArray(data.models)) {
      // ПАРНЫЕ МОДЕЛИ
      const gap = 1.5; // В полном просмотре расстояние можно сделать больше
      data.models.forEach((m, index) => {
        const offset = index === 0 ? -gap : gap;
        loader.load(m.model, (gltf) => addModelToScene(gltf, offset));
      });
      this.controls.target.set(0, 0.5, 0); // Смотрим в центр между моделями
    } else if (data.buffer) {
      // ОДИНОЧНАЯ ИЗ IDB
